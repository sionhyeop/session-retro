...
1-1. **슬롯 채우기** (`references/image-cards.md` 절차): 슬롯을 순회하며
   - 자동 생성 가능(코드/로그 카드, 스탯 카드, mermaid 다이어그램, HTML 산출물 스샷) →
     카드 HTML을 만들어 `html_shot.py`로 PNG를 뽑아 `retro/assets/auto/`에 저장하고
     슬롯 주석을 실제 이미지 참조로 교체한다.
   - 실물 스크린샷이 필요한 슬롯 → 사용자에게 "이 슬롯들은 직접 찍어서
     `retro/assets/inbox/`에 넣어주세요" 목록으로 요청하고, 못 채운 주석은 그대로 남긴다.
