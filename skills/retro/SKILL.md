...
`retro/map-actions.json`이 있으면 — 사용자가 콘텐츠 맵 UI에서 글감들을 체크해 저장한 것이다 —
**무엇보다 먼저 처리한다**: 파일을 Read하고 각 `actions[].cmd`(대개 회고 스펙 생성)를 순서대로
수행한 뒤, 파일을 삭제하고 처리 결과를 사용자에게 요약 보고한다.
...
## 6-0. 프로젝트 연대기 — 콘텐츠 맵의 축 (`retro/story.md`)

콘텐츠 맵의 메인 축은 블로그 제작 공정이 아니라 **프로젝트 자체의 시간 흐름**이다.
증류·소급 시(또는 "연대기 정리해줘" 요청 시) 인벤토리를 바탕으로 프로젝트 전체 기간을
의미 있는 **챕터**(초기 기능 구축 → 중간 기능 → 연구 주제 → 개선 작업…)로 나눠 기록한다.

- 형식 (표 한 행 = 챕터 하나, 시간순, 전체 기간을 빠짐없이 커버):

  ```markdown
  # 프로젝트 연대기
  | 기간 | 챕터 | 요약 | 글 |
  |---|---|---|---|
  | 07-01~07-05 | 초기 기능 구축 | 인증·DB 설계 | auth-struggle |
  | 07-06~07-12 | 성능 개선 | 캐시 도입, N+1 해결 |  |
  ```

- **글 칸** = 그 챕터를 다룬 에피소드 slug (비면 맵에 "✍️ 미작성"으로 노출 → 사용자가 ▶로
  바로 지시 가능). 새 에피소드가 어떤 챕터를 다루게 되면 이 칸을 갱신한다.
- 맵에서는 **같은 글을 다루는 연속 챕터가 하나의 박스로 합쳐진다** (블로그 글 하나 = 박스 하나,
  세부 챕터들은 박스 안에 "A → B → C" 흐름으로 표시) — 그러니 세부 흐름은 마음껏 잘게 기록해도 된다.
- 요약은 한 줄 — 그 시기에 무엇을 했는지 모르는 사람도 알 수 있게.
